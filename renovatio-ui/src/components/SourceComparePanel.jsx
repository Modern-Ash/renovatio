...
export default function SourceComparePanel({ projectId }) {
  ...
  const run = async () => {
    ...
    try { setResult(await compareSourceEquivalence(projectId, { caseId: `compare-${Date.now()}`, baselineSource: baseline, candidateSource: candidate, input: JSON.parse(input), files: JSON.parse(files), db2Responses: JSON.parse(db2Responses), ignoredFields: ignoredFields.split(',').map(value => value.trim()).filter(Boolean) })) }
  ...
  return <section className="architecture-preview" aria-label="Comparación de fuentes COBOL">
    ...
    <input className="source-input" value={ignoredFields} onChange={e => setIgnoredFields(e.target.value)} aria-label="Campos ignorables" placeholder="Campos ignorables: timestamp,traceId" />
