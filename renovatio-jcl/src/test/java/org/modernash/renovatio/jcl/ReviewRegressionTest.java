...
class ReviewRegressionTest {
    ...
    void flushesPendingStepBeforeStartingAnotherJob() {
        List<JclJob> jobs = new JclParser().parseAll(List.of(new org.modernash.renovatio.jcl.parse.JclSource(
    ...
    void namespacesProcedureLocalCondAndIfReferences() {
        JclJob parsed = new JclParser().parseAll(List.of(
                new org.modernash.renovatio.jcl.parse.JclSource("batch/main.jcl",
                        "//MAINJOB JOB\n//CALL EXEC PROC=FLOW\n"),
                new org.modernash.renovatio.jcl.parse.JclSource("batch/flow.jcl", """
                        //FLOW PROC
                        //EARLIER EXEC PGM=A
    ...
    void propagatesProcInvocationCondToExpandedSteps() {
        JclJob job = new JclParser().parseAll(List.of(
                new org.modernash.renovatio.jcl.parse.JclSource("batch/m.jcl",
    ...
    void sortStepWithUnsupportedControlCardClassifiesAsResidue() {
        BatchJob job = project("""
                ...
                /*
                """);
        assertEquals(org.modernash.renovatio.semantic.ir.BatchStep.Kind.RESIDUE, job.steps().get(0).kind());
    }
    ...
    void appliesProcInvocationDdOverridesToExpandedSteps() {
        JclJob job = new JclParser().parseAll(List.of(
                new org.modernash.renovatio.jcl.parse.JclSource("batch/m.jcl", """
                        ...
                        """),
                new org.modernash.renovatio.jcl.parse.JclSource("batch/gen.jcl", """
                        //GEN PROC
                        //G EXEC PGM=IEBGENER
