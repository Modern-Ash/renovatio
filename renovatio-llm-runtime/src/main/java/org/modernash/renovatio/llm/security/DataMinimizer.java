package org.modernash.renovatio.llm.security;
...
/**
 * Minimizes data sent to LLM providers by:
 * - Trimming context to essential fields only
...
public class DataMinimizer {
    ...
    // Maximum allowed string length in context
    private static final int MAX_STRING_LENGTH = 5000;
    // Maximum number of context entries
    private static final int MAX_CONTEXT_ENTRIES = 50;

    /**
     * Minimizes a context map for sending to LLM.
     */
    public Map<String, Object> minimize(Map<String, Object> context) {
        ...
        Map<String, Object> limited = limitEntries(truncated);
    ...
    private Map<String, Object> truncateStrings(Map<String, Object> context) {
        ...
        for (Map.Entry<String, Object> entry : context.entrySet()) {
            ...
            if (value instanceof String str && str.length() > MAX_STRING_LENGTH) {
                result.put(entry.getKey(), str.substring(0, MAX_STRING_LENGTH) + "...[TRUNCATED]");
            } else if (value instanceof Map) {
    ...
    private java.util.List<Object> truncateIterable(Iterable<?> iterable) {
        java.util.List<Object> result = new java.util.ArrayList<>();
        for (Object item : iterable) {
            if (item instanceof String str && str.length() > MAX_STRING_LENGTH) {
                result.add(str.substring(0, MAX_STRING_LENGTH) + "...[TRUNCATED]");
            } else if (item instanceof Map) {
    ...
    private Map<String, Object> ensureSizeLimit(Map<String, Object> context) {
        ...
        while (toJsonString(result).length() > MAX_CONTEXT_CHARS && !result.isEmpty()) {
