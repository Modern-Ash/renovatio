...
public class WorkbenchDataMigrationService {
    ...
    public WorkbenchDataMigrationDto plan(String projectId) {
        ...
        tables.forEach(table -> {
            ...
            if (table.columns().isEmpty()) {
                // A DomainNode can carry tableName/sourceDataset with zero properties
                // (the editor allows it); without this, generation would reach
                // stagingSql() and emit "CREATE TABLE ... ( );", invalid SQL
                // (see #276 review).
                gaps.add("No column mappings for " + table.domainNodeId() + "; add at least one property before generating");
            }
    ...
    private static String stagingSql(WorkbenchDataMigrationDto plan) {
        ...
        for (WorkbenchDataMigrationDto.TablePlan table : plan.tables()) {
            if (table.columns().isEmpty()) {
                // Reported as a gap in plan(); skip rather than emit invalid
                // "CREATE TABLE ... ( );" SQL (see #276 review).
                continue;
            }
