...
- Existing MCP tools remain provider-derived; `renovatio.capabilities` is appended as an additional discovery tool.
...
- MCP execution routes through `ApplicationCommandBus`; `renovatio.capabilities` returns the shared registry without provider-side projection.
