...
| Area | Spike position | Evidence required | Risk / control |
| --- | --- | --- | --- |
...
| Production dependency audit | No high or critical findings after production prune; 22 moderate findings accepted for spike verification only | `npm audit --omit=dev --json` and CI high-severity threshold | Findings are inherited from Theia (`dompurify`, `qs`, `uuid` paths); refresh the pinned Theia line and reassess before release |
...
| Commands | One Renovatio open-view command | Unit contract and manual palette/menu check | Production command allowlist and audit required |
...
This matrix supports a platform decision, not production approval. Issue #177 may resume only after
the build and runtime evidence is successful and the ADR is accepted. Production hosting and
desktop distribution require separate threat modeling and release gates.
