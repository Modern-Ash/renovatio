...
questions and stop before any decision that depends on them. Never mark criteria satisfied, grant
approval, register evidence, or transition work merely because a runtime suggested an answer.
After the user or an authorized role resolves the ambiguity in the specification, register the
current specification and rerun `agora work clarify` until its latest run leaves no unanswered
question. Then rerun `agora work traceability` to detect stale clarification provenance before
attempting the Method Pack transition.
