...
satisfied-criteria: []
criterion-statuses: {"ir-representation":["specified","planned","implemented"],"method-extraction":["specified","planned","implemented"],"perform-thru":["specified","planned","implemented"],"looping-variants":["specified","planned"],"recursion-guard":["specified","planned","implemented"],"traceability":["specified","planned","implemented"],"characterization":["specified","planned"],"nested-green":["specified","planned","implemented"],"carddemo-compiles":["specified","planned"],"regression-green":["specified","planned","implemented"]}
...
- [ ] **ir-representation:** PERFORM THRU/VARYING/UNTIL/TIMES se representan con nodos IR explícitos (paragraph, throughParagraph, varyingVariable/from/by, untilCondition, timesCount) con validación de esquema y constructor.; stages: specified, planned, implemented
- [ ] **method-extraction:** El Java generado tiene un método privado por párrafo referenciado por PERFORM, sin duplicación de cuerpo ni inlineado en el punto de llamada.; stages: specified, planned, implemented
- [ ] **perform-thru:** PERFORM p THRU q ejecuta la secuencia de párrafos p..q en orden de aparición; si el rango cruza fall-through no lineal se emite un action item.; stages: specified, planned, implemented
- [ ] **looping-variants:** PERFORM n TIMES emite for (int i=0;i<n;i++); UNTIL emite while con soporte TEST BEFORE/AFTER (default BEFORE); VARYING x FROM a BY b UNTIL cond emite for con variable de control, incluido AFTER (loops anidados); PERFORM inline (END-PERFORM) emite bloque.; stages: specified, planned
- [ ] **recursion-guard:** Recursión o ciclos entre párrafos se detectan y emiten como ManualActionItem estable; la generación nunca cuelga ni produce StackOverflowError.; stages: specified, planned, implemented
- [ ] **traceability:** Cada método extraído conserva trazabilidad @GeneratedFrom con el párrafo COBOL y su rango de líneas.; stages: specified, planned, implemented
- [ ] **characterization:** Fixtures de caracterización cubren PERFORM simple, THRU, n TIMES, UNTIL (BEFORE y AFTER) y VARYING con AFTER, con Java esperado y comportamiento.; stages: specified, planned
- [ ] **nested-green:** El fixture existente perform-simple-nested queda verde tras el cambio a métodos.; stages: specified, planned, implemented
